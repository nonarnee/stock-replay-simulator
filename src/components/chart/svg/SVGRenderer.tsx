...
export default function SVGRenderer({
  ...
  // 옵션 기본값 설정 - useMemo로 최적화
  const defaultOptions = useMemo<SVGRenderOptions>(
    () => ({
      width,
      height,
  ...
  // 옵션 병합 - useMemo로 최적화
  const mergedOptions = useMemo<SVGRenderOptions>(() => {
    return { ...defaultOptions, ...options };
  }, [defaultOptions, options]);

  // 마우스 오버 핸들러
  const handleMouseOver = useCallback((e: React.MouseEvent, dataPoint: OHLCData) => {
  ...
  // 차트 영역 계산 - mergedOptions.margin 의존성으로 변경
  useEffect(() => {
    // 디버깅용 렌더 카운트 증가
    renderCountRef.current += 1;
    console.log(`SVG rendering count: ${renderCountRef.current}`);
  ...
  }, [width, height, mergedOptions.margin]);

  // 스케일 계산 함수 - useCallback으로 최적화
  ...
  }, [data, chartArea, mergedOptions.candleWidth, mergedOptions.candleGap]);

  // 그리드 라인 생성 - useCallback으로 최적화
  const renderGridLines = useCallback(
    (scales: ReturnType<typeof calculateScales>) => {
    ...
    [chartArea, mergedOptions.showGrid, mergedOptions.gridColor, mergedOptions.showYAxis]
  );

  // X축 레이블 생성 - useCallback으로 최적화
  const renderXAxisLabels = useCallback(
    ...
    [data, chartArea, height, mergedOptions.showXAxis, mergedOptions.candleWidth]
  );

  // 캔들 차트 렌더링 - useCallback으로 최적화
  const renderCandles = useCallback(
    ...
    [
      data,
      chartArea,
      mergedOptions.upColor,
      mergedOptions.downColor,
      mergedOptions.currentTimestamp,
      mergedOptions.isAnimating,
      mergedOptions.candleWidth,
      mergedOptions.useSVGAnimation,
      handleMouseOver,
      handleMouseOut,
    ]
  );

  // 현재 가격 표시 - useCallback으로 최적화
  const renderCurrentPrice = useCallback(
    (scales: ReturnType<typeof calculateScales>) => {
      if (!scales || !data || data.length === 0) return null;
      const { yScale } = scales;
    ...
    [data, chartArea, width, mergedOptions.currentTimestamp]
  );

  // 계산된 스케일 메모이제이션
  const scales = useMemo(() => calculateScales(), [calculateScales]);
      ...
      <StyledSVG ref={svgRef} width={width} height={height} className={mergedOptions.svgClassName}>
        {scales && (
          <>
            ...
            {renderCandles(scales)}
            {renderXAxisLabels(scales)}
            {renderCurrentPrice(scales)}
          </>
        )}
      </StyledSVG>
...
SVGRenderer.render = (data: OHLCData[], options: SVGRenderOptions): void => {
  // React 컴포넌트로 렌더링은 React에서 처리
  console.log('Rendering SVG with data length:', data.length, 'and options:', options);
};
